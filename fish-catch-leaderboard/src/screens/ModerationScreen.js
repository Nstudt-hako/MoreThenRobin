...
const ModerationScreen = () => {
  ...
  const load = async () => {
    ...
    setError("");
    try {
      const data = await queryCatches(
        (c) => (!c.verified || c.flagged) && !c.isDeleted
      );
      setPending(data);
    } catch (e) {
  ...
  const doFlag = async (id) => {
    try {
      await flagCatch(id, "manual-flag");
      await load();
    } catch {
      /* ignore */
    }
  ...
  if (!isAdmin) {
    return (
      <div style={{ padding: "2rem" }}>
        <h1 style={{ color: "var(--color-primary)" }}>Moderation</h1>
  ...
  return (
    <div style={{ padding: "2rem" }}>
      <h1 style={{ color: "var(--color-primary)" }}>Moderation Queue</h1>
      {loading && <p>Loading...</p>}
