...
const LeaderboardScreen = () => {
    ...
    const [filters, setFilters] = useState({ species: '', timeframe: 'all', search: '', sort: 'size', onlyVerified: true, page: 1, pageSize: 10 });

    const updateFilter = (e) => {
        const { name, value, type, checked } = e.target;
    setFilters(prev => ({ ...prev, [name]: type === 'checkbox' ? checked : value, page: name === 'pageSize' ? 1 : (name==='species'||name==='timeframe'||name==='sort'||name==='search'||name==='onlyVerified'?1:prev.page) }));
    };
    ...
    useEffect(() => {
        const fetchLeaderboardData = async () => {
            setLoading(true);
            try {
                const result = activeGroupId ? await loadGroupLeaderboard(filters) : await filterLeaderboard(filters);
                setLeaderboardData(result.data);
                setPageInfo({ page: result.page, total: result.total, pageSize: result.pageSize });
            } catch (error) {
                setLeaderboardData([]);
            } finally {
                setLoading(false);
    ...
    const goPage = (p) => setFilters(prev => ({ ...prev, page: Math.min(Math.max(1, p), totalPages) }));
    ...
    return (
        <div className="leaderboard-container" style={{ backgroundColor: theme.background }}>
            <h1 className="leaderboard-title" style={{ color: theme.text }}>🏆 {activeGroupId ? 'Group' : 'Global'} Leaderboard</h1>
            <div className="filter-bar" style={{ marginBottom: '1.5rem', display: 'flex', flexWrap: 'wrap', gap: '0.75rem' }}>
                ...
                <select name="pageSize" value={filters.pageSize} onChange={updateFilter}>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:'1rem', gap:'1rem', flexWrap:'wrap' }}>
                <div style={{ fontSize:'0.85rem', opacity:0.8 }}>Page {pageInfo.page} / {totalPages} • Total {pageInfo.total}</div>
                <div style={{ display:'flex', gap:'0.5rem' }}>
                    <button className="btn-secondary" disabled={filters.page<=1 || loading} onClick={()=>goPage(filters.page-1)}>Prev</button>
                    <button className="btn-secondary" disabled={filters.page>=totalPages || loading} onClick={()=>goPage(filters.page+1)}>Next</button>
                </div>
            </div>
        </div>
    );
